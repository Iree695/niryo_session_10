"""
Full training pipeline for dice face classifier. Orchestrates data loading,
model training with logging, evaluation, and ONNX export for deployment.

Critical: Image loading/normalization must match classifier.py preprocessing exactly.
"""
...
def set_seed(seed: int) -> None:
    """Establish reproducible RNG state across libraries."""
...
class DiceDataset(Dataset):
    """Lazy-load BGR crops from disk to avoid memory overhead during training."""
    ...
    def __getitem__(self, idx):
        # OpenCV loads in BGR uint8 to maintain consistency with runtime inference pipeline
        img = cv2.imread(self.image_paths[idx])
        img = cv2.resize(img, CNN_INPUT_SIZE)       # standardize spatial dimensions
        img = img.astype(np.float32) / 255.0        # normalize to [0, 1]
        img = torch.from_numpy(img.transpose(2, 0, 1))  # change layout to CHW
...
def load_dataset():
    """Recursively index all training samples from label subdirectories."""
    paths = []
    labels = []
...
class DiceCNN(nn.Module):
    """Three-block CNN with batch normalization and progressive spatial reduction."""

    def __init__(self, num_classes=NUM_CLASSES):
        super().__init__()
        # Downsampling block: 3x64x64 -> 32x32x32
        ...
        # Downsampling block: 32x32x32 -> 64x16x16
        ...
        # Downsampling block: 64x16x16 -> 128x8x8
        ...
        # Classifier head: fully connected with dropout regularization
