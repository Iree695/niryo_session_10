"""
Interactive visualization and command dispatcher for robot control.
Renders real-time detection overlays and processes terminal input.
"""
...
def apply_filters(objects: List[Dict], color_filter: str, shape_filter: str) -> List[Dict]:
    """Reduce object list to those matching color and/or shape predicates."""
    filtered = objects
    if color_filter != "ANY":
...
def draw_workspace_overlay(frame: np.ndarray, corners: np.ndarray, detected_live: bool) -> None:
    """Render calibration frame boundary with corner labels and detection status."""
    ...
    # Mark calibration fiducial centers
    ...
    status = "WS: fiducials live" if detected_live else "WS: fallback (no fiducials)"
...
def draw_objects(frame: np.ndarray, objects: List[Dict], selected_index: Optional[int]) -> None:
    """Draw bounding boxes with centroids and class labels. Highlight selected object."""
...
def draw_hud(frame: np.ndarray, picker: NiryoVisionPicker, visible_objects: List[Dict]) -> None:
    """Display control instructions and system state overlay."""
    lines = [
...
def print_terminal_help() -> None:
    """List all available robot control commands."""
...
def input_worker(cmd_queue: "queue.Queue[str]") -> None:
    """Background thread: read user commands from terminal and queue for main loop."""
    while True:
        try:
