"""
Computer vision stack for workspace detection and object localization.
Coordinates transformations between image-space and robot-space coordinates.
"""
...
def fallback_workspace_corners() -> np.ndarray:
    """Return hardcoded workspace bounds if active fiducial detection fails."""
    ...
    # Image corner reference points (TL, TR, BR, BL)
    return np.array([
        [x_min, y_min],
        [x_max, y_min],
        [x_max, y_max],
...
def detect_workspace_from_dianas(frame: np.ndarray) -> Optional[np.ndarray]:
    """Locate calibration fiducials (circular markers) and map to workspace corners.
    Returns None if detection fails robustness checks."""
    ...
    # Image corner anchors for greedy nearest-neighbor assignment
    ...
    # Sanity check: inferred workspace area must exceed minimum threshold
...
def point_inside_workspace(point: Tuple[int, int], corners: np.ndarray) -> bool:
    """Test if 2D point lies within workspace polygon boundary."""
...
def pixel_to_relative(px: int, py: int, corners: np.ndarray) -> Tuple[float, float]:
    """Warp pixel coordinates to normalized workspace-local [0..1]x[0..1] via perspective transform."""
    src = corners.astype(np.float32)
    dst = np.array([[0, 0], [1, 0], [1, 1], [0, 1]], dtype=np.float32)
    H = cv2.getPerspectiveTransform(src, dst) # homography matrix
...
def classify_shape(num_vertices: int, aspect_ratio: float, circularity: float) -> str:
    """Discriminate geometric shape using contour properties and form factor."""
    if circularity > 0.85:
        return "CIRCLE"
...
def detect_color(image: np.ndarray, contour: np.ndarray) -> str:
    """Sample average color within region-of-interest and match against HSV thresholds."""
...
def preprocess(frame: np.ndarray) -> np.ndarray:
    """Apply HSV thresholding and morphological cleanup to extract object candidates."""
...
def detect_objects(frame: np.ndarray, workspace_corners: np.ndarray) -> List[Dict]:
    """Segment objects by color and shape within workspace bounds. Returns list sorted by area."""
